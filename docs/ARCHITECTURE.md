...
| 4 | ~~InvoiceList has an inline modal~~ Generic `Modal` primitive built (`ConfirmModal` uses it); `InvoiceList`'s "Mark Paid" inline modal still not migrated | Use the shared `Modal` primitive | Low |
